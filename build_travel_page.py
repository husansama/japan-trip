...
SEGMENT_ROUTES: dict[str, list[dict[str, str]]] = {
    ...
    "Day7": [
        segment("喜织轻旅酒店五条通 KIORI Exec Gojo", "京都站", "约10–20分钟", "公共交通", note="堀川五条乘公交到京都站；任天堂博物馆已订16:30，早上不必过早出门", map_start="KIORI Exec Gojo Kyoto", map_end="Kyoto Station"),
        ...
        segment("JR宇治站", "JR小仓站", "约3分钟", "公共交通", note="JR奈良线一站；也可从宇治沿宇治川慢走后再搭车", map_start="Uji Station JR", map_end="Ogura Station JR Kyoto"),
        segment("JR小仓站", "任天堂博物馆", "约8分钟", "步行", note="已订10月1日16:30入馆，建议最迟16:00抵达并备好护照和二维码", map_start="Ogura Station JR Kyoto", map_end="Nintendo Museum Kyoto"),
        segment("任天堂博物馆", "JR小仓站", "约8分钟", "步行", note="闭馆20:00；互动体验与商店结束后原路返回", map_start="Nintendo Museum Kyoto", map_end="Ogura Station JR Kyoto"),
        segment("JR小仓站", "京都站", "约25–35分钟", "公共交通", note="JR奈良线返回；晚餐在京都站或酒店附近解决", map_start="Ogura Station JR Kyoto", map_end="Kyoto Station"),
    ],
    "Day8": [
...
DAY_OVERRIDES: dict[str, dict[str, str]] = {
    ...
    "Day3": {
        ...
        "晚餐/夜间建议": "MOURIYA神户牛午餐已预约；南京町只补少量小吃，傍晚到Harborland看夜景并吃轻晚餐",
    },
    "Day4": {
    ...
    "Day6": {
        ...
        "交通重点": "KIORI→京都站：早班小火车日优先短程打车，预算优先再乘公交；之后京都站→JR马堀→小火车龟冈，午后岚电岚山站经帷子之辻到龙安寺。",
    ...
    "Day7": {
        ...
        "P人友好玩法": "宇治只安排半天，午后留给河边散步、咖啡或休息；15:45前抵JR小仓站，16:00前到博物馆。",
        "交通重点": "KIORI先到京都站，JR快速到宇治；从JR宇治乘一站到JR小仓，再步行8分钟。博物馆结束后原路回京都站。",
        "晚餐/夜间建议": "博物馆可玩至闭馆20点；回京都站或堀川五条附近吃晚餐，不再强行塞奈良。",
    },
    "Day8": {
...
DAY_ATTRACTION_GUIDES: dict[str, list[dict[str, str]]] = {
    ...
    "Day7": [
        {"名称": "平等院凤凰堂", "建议停留": "60–75分钟", "说明": "十日元硬币背面的凤凰堂是宇治最重要的建筑；庭园与水面倒影比赶周边店铺更值得。", "怎么玩": "先逛庭园和凤凰堂外观，博物馆按兴趣进入；下午有固定预约，因此内部参观排队过长就放弃。", "取舍": "宇治只是上午轻量站，不延伸太多博物馆或购物。"},
        {"名称": "宇治川与抹茶街", "建议停留": "90–150分钟", "说明": "宇治的价值是河边散步和抹茶文化，不是连续排队买甜品；本日午后有充裕缓冲。", "怎么玩": "沿宇治桥和河岸走一小段，选一家不排队的茶屋吃早午餐或甜品，15:15前往JR小仓方向移动。", "取舍": "排队超过20分钟就换店；宁可留出入馆前缓冲，也不要卡点赶博物馆。"},
        {"名称": "任天堂博物馆（已订16:30）", "建议停留": "约3小时", "说明": "以任天堂产品、开发史和可动手玩的互动体验为核心，适合把本次京都行程从纯看景补成真正的参与型项目。", "怎么玩": "16:00前抵达，备好护照与电子票；进馆后先确认互动体验，再看展与商店。体验卡有10枚虚拟金币，按兴趣分配，不必每一项都排。", "取舍": "16:30是当日硬锚点，宇治任何排队项目都应为它让路；工作坊若有当日名额再加，不把它作为必做。"},
    ],
    "Day8": [
...
DAY_DETAIL_OVERRIDES: dict[str, list[dict[str, str]]] = {
    ...
    "Day3": [
        ...
        day_detail("09-27", "以预约时间为准", "抵神户三宫→MOURIYA神户牛午餐（已预约）→南京町→旧居留地", "姬路城排队过长就删好古园，直接前往三宫", "神户牛预约是当天硬节点，午餐后沿元町向港湾单向推进最顺", "以TableCheck确认页的具体入店时间倒推；最晚提前15分钟到店，若JR延误立即电话联系餐厅"),
    ...
    "Day7": [
        day_detail("10-01", "08:00–08:50", "KIORI→京都站→JR快速到宇治，先在宇治川周边散步", "8:30前后出发也可", "平等院庭园8:45开放，早到不排队", "博物馆已固定16:30，上午不必硬赶首班车"),
        day_detail("10-01", "08:50–12:00", "平等院庭园＋博物馆→抹茶街早午餐/甜品", "不进凤凰堂内部", "把平等院作为宇治的唯一硬景点", "凤凰堂内部票需现场定时，等待长就跳过"),
        day_detail("10-01", "12:00–15:15", "宇治桥/宇治川散步→茶屋或咖啡→按体力休息", "缩短为宇治川散步＋一顿抹茶", "用充裕午后吸收排队与休息，不再跨城去奈良", "15:15前结束宇治市区活动，准备向JR小仓移动"),
        day_detail("10-01", "15:15–16:00", "JR宇治→JR小仓→步行至任天堂博物馆、办理入馆准备", "从宇治直接打车到博物馆", "16:30预约不可迟到，预留至少30分钟机动比多逛一间茶屋重要", "电子票、护照和手机电量提前准备；以票面入馆规则为准"),
        day_detail("10-01", "16:30–20:00", "任天堂博物馆展览→互动体验→商店→JR小仓回京都", "互动区优先、展览按兴趣压缩", "已订票是京都行程中最稀缺的互动体验，完整留出三小时以上", "互动体验使用入馆发放的游戏卡；工作坊如要参加，入馆后尽早确认当日名额与时间"),
    ],
    "Day8": [
...
DAY_FOOD_OVERRIDES: dict[str, dict[str, str]] = {
    ...
    "Day3": {
        "午餐": "MOURIYA三宫店神户牛午餐已预约；姬路城后只补轻食，按TableCheck确认页时间抵店，别在姬路吃正餐拖慢转场。",
        "晚餐": "神户牛后的港湾收尾三选一：最推荐MOSAIC 2F花水木点一份出石荞麦；不饿就KANNONYA芝士甜点＋咖啡；午餐吃得太少才在南京町补一份小吃后去Harborland。",
        "P人吃法": "神户牛已订，不再准备Steakland备选；午餐已吃牛排就不要在港湾再点牛排、烧肉或海鲜自助，目标是19:30–20:00离开神户。",
    },
    "Day4": {
    ...
    "Day7": {
        "早餐/早午餐": "平等院后再去宇治桥通吃抹茶早午餐；中村藤吉本店不接受咖啡预约，若等待超过30分钟改伊藤久右卫门或通圆茶屋。",
        "午餐": "宇治把抹茶荞麦、茶泡饭或甜品当早午餐，13:30前结束；14:15–15:00在宇治提前吃一顿轻晚餐，避免博物馆闭馆后才找餐厅。",
        "晚餐": "任天堂博物馆至20:00后只补便利店／京都站可见仍营业的简餐；不要把正式晚饭赌在20:30后的京都站。",
        "甜品/咖啡/夜宵": "宇治只留一份抹茶甜品；若中村藤吉排队长，改通圆茶屋或伊藤久右卫门，不再额外跑奈良甜品。",
        "P人吃法": "16:30博物馆是硬预约，任何茶屋排队都要让路；提前吃饱、带水，博物馆结束后直接回酒店。",
    },
    "Day8": {
...
COUPLE_FOOD_GUIDE: list[dict[str, str]] = [
    ...
    {"日期": "Day3", "地点": "神户 · 已预约神户牛", "主题": "一顿主角餐，换一个港湾黄昏", "主选": "按预约吃 MOURIYA；晚上 Harborland 只吃荞麦、甜品或轻食。", "氛围": "午餐拍铁板与两人举杯；黄昏拍港口，不需再吃第二顿牛排。", "避坑": "姬路延误就舍弃好古园；别让景点挤压已预约午餐。"},
    ...
    {"日期": "Day6", "地点": "京都 · 岚山", "主题": "庭园与河边比竹林更松弛", "主选": "优先用岚山美食券；若不适用再选岚山よしむ荞麦，晚上京都站拉面小路。", "氛围": "拍渡月桥、河岸与和服之森；咖啡只外带，不坐等网红店。", "避坑": "小火车延误就删咖啡或龙安寺，不为拍竹林久等。"},
    {"日期": "Day7", "地点": "宇治 · 任天堂博物馆", "主题": "抹茶是点缀，不是排队任务", "主选": "中村藤吉排队长即换伊藤久右卫门或通圆；15:15 前在宇治吃轻晚餐。", "氛围": "平等院倒影、宇治川和一份抹茶甜品即可；博物馆先互动拍照、后购物。", "避坑": "16:30 博物馆是硬预约，任何茶屋都要给它让路。"},
