...
const DAILY_POSTERS = [
  // 使用穿搭版海报展示每日行程；原图仍保存在 posters 中，便于需要时回退或对比。
  ["day1-kansai-arrival-outfit.png", "Day 1 · 关西抵达日", "香港 → 关西机场 → 心斋桥"],
  ["day2-osaka-city-outfit.png", "Day 2 · 大阪市区慢游", "大阪城 → 历史馆 → 梅田 → 中之岛 → 难波"],
  ["day3-usj-outfit.png", "Day 3 · 主题乐园", "游戏区域 → 魔法城堡 → 夜间灯光"],
  ["day4-kobe-watercolor-outfit.png", "Day 4 · 神户海岸线", "须磨 → 须磨浦 → 舞子 → 三宫港区"],
  ["day5-kyoto-higashiyama-outfit.png", "Day 5 · 京都东山", "清水寺 → 祇园 → 鸭川"],
  ["day6-arashiyama-kinkakuji-outfit.png", "Day 6 · 岚山与金阁寺", "小火车 → 竹林 → 金阁寺"],
  ["day7-fushimi-uji-nintendo-outfit.png", "Day 7 · 伏见稻荷与宇治", "鸟居 → 抹茶 → 任天堂博物馆"],
  ["day8-nara-outfit.png", "Day 8 · 奈良慢游", "鹿 → 东大寺 → 春日大社 → 奈良町"],
  ["day9-tokyo-arrival-outfit.png", "Day 9 · 新干线进东京", "京都 → 新桥 → 东京铁塔"],
  ["day10-shibuya-haneda-outfit.png", "Day 10 · 涩谷与羽田", "明治神宫 → 涩谷SKY → 羽田"],
  ["day11-haneda-return-outfit.png", "Day 11 · 羽田返程日", "羽田 → 北京中转 → 广州"],
];
...
function posterAssetPath(fileName) {
  const inTravelPage = window.location.pathname.replace(/\\/g, "/").includes("/travel-page/");
  return `${inTravelPage ? "../" : "./"}posters/${fileName}`;
}

function renderPosters() {
  $("#posterGallery").innerHTML = DAILY_POSTERS.map(([fileName, title, route]) => {
    const asset = posterAssetPath(fileName);
    return `
      <a class="poster-card" href="${asset}" target="_blank" rel="noopener noreferrer" title="打开${title}原图">
        <img src="${asset}" alt="${title}行程海报" loading="lazy" />
        <div class="poster-card-copy"><strong>${title}</strong><span>${route}</span></div>
      </a>
    `;
  }).join("");
}
...
function renderReview() {
  const dimensions = [
    ["景点含金量", "4.6", "舞子海上步道、须磨虎鲸、京都经典线、USJ与任天堂博物馆共同拉高上限"],
    ...
    ["交通顺路度", "4.2", "Day4先锁定须磨双表演，再向西串须磨浦与舞子日落；虽有一次换乘，但体验层次更完整"],
  ];
  const days = [
    ["Day2 · 大阪市区", "4.1", "大阪城只保留御座船；道顿堀夜船改为¥900自费备选，晚间可更从容地吃饭、拍夜景。"],
    ["Day3 · USJ", "4.4", "互动与情侣共同满意度最高；水世界与SING只选一场，用于坐下恢复体力。"],
    ["Day4 · 神户一日", "4.6", "须磨双表演、须磨浦缆车、舞子日落和神户牛形成完整节奏；前提是神户牛改至19:15。"],
  ...
  const groups = [
    ["Day2", [["大阪城外观＋御座船", "3.8／3.8／3.0／4.2", "与姬路城同属城堡", "只看外观，不进天守阁"], ["大阪历史博物馆", "4.0／3.6／3.2／4.6", "低", "城堡之后的室内缓冲，延误时可删"], ["国立国际美术馆", "4.2／4.0／3.2／4.5", "与寺社、商场不重复", "9月26日免费日，控制在90分钟内"], ["难波八阪神社＋法善寺横丁", "4.0／4.6／2.8／4.4", "低", "拍狮子殿后接横丁与道顿堀"], ["心斋桥／道顿堀夜逛", "4.1／4.4／3.0／4.2", "低", "保留，是大阪的代表性夜景"]]],
    ...
    ["Day4", [["须磨海洋公园", "4.7／4.4／4.7／3.8", "低", "11:00与11:45双表演优先，13:40硬离园"], ["须磨浦山上游园A线", "4.2／4.7／4.4／3.9", "与舞子同看海峡但视角不同", "用90分钟完整回游，14:20后降为B线"], ["舞子海上步道＋日落", "4.5／5.0／3.8／4.1", "低", "步道拍45分钟，17:05转海边等17:48日落"], ["MOURIYA 神户牛晚餐", "4.7／4.4／4.5／4.8", "低", "需改约19:15左右；晚餐拖延就只拍美利坚公园"]]],
  ...
  const repetitions = [["城堡", "大阪城 vs 姬路城", "大阪城只保留外观与御座船；姬路城作为唯一正式入内城堡。"], ["游船", "大阪城御座船 vs 道顿堀夜船", "普通三选一保留价值更高的御座船；道顿堀夜船按当晚体力决定。"], ["京都寺社／庭园", "好古园、天龙寺、金阁寺、平等院", "龙安寺已删除；Day4时间紧时再删除好古园，避免庭园审美疲劳。"], ["神社", "难波八阪、八坂、伏见稻荷、春日、明治", "难波八阪只作Day2短拍；伏见和明治保留；春日按Day8体力决定。"], ["老街", "二年坂、祇园、奈良町", "东山是主角；祇园快走，奈良町与春日二选一。"], ["高空夜景", "东京铁塔地面拍摄 vs 涩谷SKY", "一个是地标合照、一个是城市俯瞰，视角不同且分两天安排，不算重复。"], ["任天堂", "USJ超级任天堂世界 vs 任天堂博物馆", "不重复：前者是游乐园互动，后者是历史展与室内互动，两个都保留。"]];
...
function viewFromHash() {
  ...
  const validViews = ["days", "posters", "review", "food", "lodging", "usj", "planb", "prep"];
